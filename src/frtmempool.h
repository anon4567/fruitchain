...
#ifndef BITCOIN_FRTMEMPOOL_H
#define BITCOIN_FRTMEMPOOL_H
...
static const unsigned int MEMPOOL_HEIGHT_FRUIT = 0x7FFFFFFF;
/*TODO seems to be unnessary for fruit
struct FruitLockPoints
{
    ...
    FruitLockPoints() : height(0), time(0), maxInputBlock(NULL) { }
};*/
...
{
    update_lock_points_fruit(const FruitLockPoints& _lp) : lp(_lp) { }

    void operator() (CFrtMemPoolEntry &e) { e.UpdateLockPoints(lp); }

private:
    const FruitLockPoints& lp;
};*/
...
public:
    ...
    typedef indexed_fruit_set::nth_index<0>::type::iterator frtiter;
